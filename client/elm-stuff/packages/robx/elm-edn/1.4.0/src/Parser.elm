...
You get:
  ...
  - A stack of `context` that describes where the error is _conceptually_.

**Note:** `context` is a stack. That means [`inContext`](#inContext)
adds to the _front_ of this list, not the back. So if you want the
...
occurred. But what if you could _also_ say “the error occured **while
parsing a list**” and let folks know what the _parser_ thinks it is
doing?!
...
{-| Run a parser _and then_ run another parser!
-}
...
move on and try the next one. If a parser _does_ commit, we give up on any
remaining parsers.

The idea is: if you make progress and commit to a parser, you want to
get error messages from _that path_. If you bactrack and keep trying stuff
...
`[` we _commit_ to that parser. That means if something goes wrong, we do
...
final string. That means _three_ strings are allocated.
...
This version only allocates _one_ string.
...
{-| Ignore characters until _after_ the given string.
